    ...
    public static class SupervisorCallCollection
    {
        public static void Call(int ID, ObjectIndexer<ulong> Registers)
        {
            if (Calls[ID] != null)
            {
                ...
                Debug.Log($"Called { Calls[ID].Method.Name} From {GetThread(Registers).ID}",LogLevel.Low);
            }
            else
            {
        ...
        static Dictionary<int, SupervisorCall> Calls { get; set; } = new Dictionary<int, SupervisorCall>()
        {
            ...
            { 0x2A, null },
            { 0x2B, null },
            { 0x2C, SvcMemory.MapPhysicalMemory },
            { 0x2D, null },
            { 0x2E, null },
            { 0x2F, null },
            { 0x30, null },
        ...
        public static void SvcLog(string message) => Debug.Log($"SVC: {message}",LogLevel.Neutral);
